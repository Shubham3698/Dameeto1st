...
export default function StickerAdmin({ onClose, API_BASE_URL }) {
  ...
  // fetchGroups ko useCallback mein wrap kiya taaki cascade render na ho
  ...
  const handleUpload = async (targetCategory = category) => {
    ...
    try {
      ...
      setTempFiles([]);
      if (tab === "edit") {
        fetchGroupDetails(targetCategory);
      } else {
        setTab("manage");
      }
    } catch (err) { 
  ...
  return (
    <div style={s.overlay}>
      <div style={s.card}>
        ...
        {tab === "upload" && (
          <div>
            <input style={{width:'100%', padding:'10px', background:'#0f172a', color:'white', border:'1px solid #334155', marginBottom:'10px'}} value={category} onChange={(e)=>setCategory(e.target.value)} placeholder="GROUP NAME"/>
            <input type="file" multiple onChange={(e)=>setTempFiles(Array.from(e.target.files).map(f=>({file:f, preview:URL.createObjectURL(f)})))} />
            <div style={s.grid}>{tempFiles.map((f,i)=><img key={i} src={f.preview} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="p"/>)}</div>
            <button onClick={()=>handleUpload()} disabled={loading} style={{width:'100%', padding:'12px', background:'#fe3d00', color:'white', border:'none', borderRadius:'8px', fontWeight:'bold'}}>{loading ? "UPLOADING..." : "SAVE GROUP"}</button>
          </div>
        )}
        ...
        {tab === "manage" && (
          <div>
            ...
            {groups.map((g, i) => (
              <div key={i} style={{display:'flex', justifyContent:'space-between', background:'#0f172a', padding:'10px', marginBottom:'10px', borderRadius:'10px'}}>
                <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
                  <img src={g.preview} style={{width:'40px', height:'40px', objectFit:'cover'}} alt="p"/>
                  <span>{g.name}</span>
                </div>
                <button onClick={()=>fetchGroupDetails(g.name)} style={{background:'#fe3d00', border:'none', color:'white', padding:'5px 15px', borderRadius:'5px', cursor:'pointer'}}>Edit Items</button>
              </div>
            ))}
          </div>
        )}
        ...
        {tab === "edit" && (
          <div>
            <div style={{display:'flex', justifyContent:'space-between', marginBottom:'15px'}}>
              <h3 style={{color:'#fe3d00', margin:0}}>Editing: {viewingGroup}</h3>
              <button onClick={()=>setTab("manage")} style={{background:'none', border:'none', color:'#94a3b8', cursor:'pointer'}}>Back</button>
            </div>
            
            <div style={{background:'#0f172a', padding:'10px', borderRadius:'10px', marginBottom:'20px'}}>
              <p style={{fontSize:'12px', margin:'0 0 10px 0'}}>Add images to {viewingGroup}:</p>
              <input type="file" multiple onChange={(e)=>setTempFiles(Array.from(e.target.files).map(f=>({file:f, preview:URL.createObjectURL(f)})))} />
              {tempFiles.length > 0 && <button onClick={()=>handleUpload(viewingGroup)} style={{width:'100%', marginTop:'10px', background:'#22c55e', border:'none', color:'white', padding:'8px', borderRadius:'5px'}}>Upload New Items</button>}
            </div>
            ...
            <div style={s.grid}>
              {selectedGroupItems.map((img) => (
                <div key={img._id} style={s.imgBox}>
                  ...
                  <button onClick={()=>deleteSingleSticker(img._id)} style={{position:'absolute', top:2, right:2, background:'red', color:'white', border:'none', borderRadius:'50%', width:'20px', height:'20px', fontSize:'10px', cursor:'pointer'}}>X</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <button onClick={onClose} style={{width:'100%', marginTop:'20px', background:'none', border:'none', color:'#64748b', cursor:'pointer'}}>Close Admin</button>
      </div>
    </div>
  );
}