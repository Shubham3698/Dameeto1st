...
export default function Nwmasonry({ images = [], categoryName }) {
  ...
  return (
    <div
      ...
      style={{ perspective: "1000px", scrollBehavior: "auto" }}
    >
            ...
            >
              ...
              {item.badge && (
                <div className={`absolute left-[6px] top-[6px] z-10 flex h-[18px] w-[30px] items-center justify-center overflow-hidden rounded-lg text-[7px] font-semibold uppercase tracking-wider text-white animate-rgb-flow transition-opacity duration-300 ${isTouch ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
                  <div className={`whitespace-nowrap ${item.badge.length > 4 ? "animate-scroll-text pl-[100%]" : ""}`}>
                    {item.badge}
                  </div>
                </div>
              )}
              ...
              <img
                ...
                className={`w-full rounded-[14px] border-none shadow-none transition-all duration-500 ease-out ${
                  isTouch
                  ? "scale-[1.1] shadow-2xl shadow-black/30 grayscale-0 opacity-100 z-50"
                  : "grayscale-[30%] opacity-90 group-hover:scale-[1.1] group-hover:shadow-2xl group-hover:shadow-black/30 group-hover:grayscale-0 group-hover:opacity-100 group-hover:z-50"
                }`}
              />
