import Prgg from "../assets/prg.jpg";

// import Prgg from "../assets/prg.jpg";

export const stickerData = [
  {
    id: "st-1",
    category: "stickers",
    src: "https://i.pinimg.com/1200x/5a/ad/0a/5aad0a8534ff652ffab1788930547741.jpg",
    subImages: [],
    title: "Girl Aesthetic Sticker",
    tag: "Aesthetic",
    badge: "hot",
    shortDesc: "Premium glossy sticker.",
    longDesc: "This aesthetic girl sticker expresses mood, personality, vibes and looks great on phone covers and laptops.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "st-2",
    category: "stickers",
    src: "https://i.pinimg.com/736x/e2/c4/13/e2c41353d25e9aaaaeff36bbf125beea.jpg",
    title: "Dark Anime Sticker",
    tag: "Anime",
    shortDesc: "High-quality matte finish.",
    longDesc: "Perfect for notebooks, laptops and diaries. Stylish expressive design representing emotions and individuality.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "st-3",
    category: "stickers",
    src: "https://i.pinimg.com/1200x/8b/0c/14/8b0c14adee9fc943bc139a125a0fc400.jpg",
    title: "Moody Girl Art Sticker",
    tag: "Moody",
    shortDesc: "Top trending artistic design.",
    longDesc: "Crafted with aesthetic details, ideal for personal expression and daily vibe representation.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "st-4",
    category: "stickers",
    src: "https://i.pinimg.com/736x/f1/02/64/f102649980fc75461d4d78239847772a.jpg",
    title: "spider-man",
    tag: "sticker",
    shortDesc: "Top trending artistic design.",
    longDesc: "Crafted with aesthetic details, ideal for personal expression and daily vibe representation.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "st-5",
    category: "stickers",
    src: "https://i.pinimg.com/1200x/80/2c/6d/802c6df0611780efb775913bc18ad73a.jpg",
    title: "Modern Cool Girl Sticker",
    tag: "Modern",
    shortDesc: "Premium waterproof sticker.",
    longDesc: "Designed for expressing modern personality and fashion sense on your accessories.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "st-6",
    category: "stickers",
    src: "https://i.pinimg.com/736x/c5/31/c3/c531c3a450cb52d496b132bea6dee5fb.jpg",
    title: "Cute Cat Sticker",
    tag: "Cute",
    shortDesc: "Adorable pet-themed sticker.",
    longDesc: "Made for fun and cuteness lovers, perfect for diaries, bottles, laptops, and more.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "st-7",
    category: "stickers",
    src: "https://i.pinimg.com/736x/59/25/04/5925040a9c9e5814e66af9e93ffb31ff.jpg",
    title: "Sad Anime Boy Sticker",
    tag: "Sad Anime",
    shortDesc: "Emotional character design.",
    longDesc: "Shows deep emotions and relatable feelings. Best for mood expression and aesthetics.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "st-8",
    category: "stickers",
    src: "https://i.pinimg.com/736x/01/c2/ad/01c2ad7a7c93fc6508ed6753daf04455.jpg",
    title: "Vibes Sticker Art",
    tag: "Vibes",
    shortDesc: "Minimalist expressive sticker.",
    longDesc: "Represents personal vibes, aesthetic energy, and clean graphic style.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "st-9",
    category: "stickers",
    src: "https://i.pinimg.com/736x/91/a2/1e/91a21e1ae32dd8f7d353f13bcac7cd07.jpg",
    title: "Cool Character Sticker",
    tag: "Cool",
    shortDesc: "Trendy new-gen design.",
    longDesc: "Designed for modern youth to reflect individuality, emotions, and personality.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "st-10",
    category: "stickers",
    src: "https://i.pinimg.com/1200x/da/d0/ae/dad0ae19bc31931008317564ba5ff832.jpg",
    title: "Soft Aesthetic Girl Sticker",
    tag: "Soft",
    shortDesc: "Soft pastel tone sticker.",
    longDesc: "Represents soft emotions, calm vibes and smooth aesthetic art style.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "st-11",
    category: "stickers",
    src: "https://i.pinimg.com/736x/0c/15/a0/0c15a0ac00f2242e18148d374cb89cdc.jpg",
    title: "Soft Aesthetic Girl Sticker",
    tag: "Soft",
    shortDesc: "Soft pastel tone sticker.",
    longDesc: "Represents soft emotions, calm vibes and smooth aesthetic art style.",
    originalPrice: 149,
    finalPrice: 99,
  },
];

export const trendingData = [
  {
    id: "tr-1",
    category: "trending",
    title: "Dark Anime Aesthetic Poster",
    shortDesc: "Premium matte finish wall poster",
    longDesc: "High quality HD anime aesthetic poster printed on thick premium paper with vibrant long-lasting colors.",
    finalPrice: 1,
    originalPrice: 299,
    discount: 33,
    rating: 4.5,
    stock: 10,
    src: "https://i.pinimg.com/736x/64/5d/4b/645d4bb5ef2069c6ef72f9e112272d14.jpg",
    subImages: [
      "https://i.pinimg.com/1200x/3e/55/b4/3e55b4dd734305304829a190c4f8cec5.jpg",
      "https://i.pinimg.com/736x/0b/e1/17/0be11763788d6da33ddc96a52e1b320b.jpg",
      "https://i.pinimg.com/736x/c7/32/e7/c732e7f11dd3c7eb110acbda090f8d5f.jpg"
    ],
    videoUrl: "https://youtu.be/4UcG9pDQ5Ss",
    tags: ["anime", "dark", "aesthetic","ytt"],
  },
  {
    id: "tr-2",
    category: "trending",
    src: "https://i.pinimg.com/736x/03/a5/7d/03a57dcb9c8d55026d079cc4db3d2265.jpg",
    title: "Lucia Jason with pistol",
    tag: "Trending GTA VI ",
    badge: "New",
    shortDesc: "Lucia Caminos | Jason Duval DTF-sticker",
    longDesc: "This Lucia Caminos | Jason Duval They are believed to be the main protagonists of the upcoming game Grand Theft Auto VI. DTF sticker (6 × 4 cm) reflects a bold, rebellious duo vibe — a mix of loyalty, ambition and fearless street energy.",
    originalPrice: 199,
    finalPrice: 29,
  },
  {
    id: "tr-3",
    category: "trending",
    src: "https://i.pinimg.com/736x/94/9b/e9/949be92bdc0fce7693370d4c340df5ec.jpg",
    title: "Lucia Jason with pistol",
    tag: "Trending GTA VI ",
    badge: "trending",
    shortDesc: "Lucia Caminos | Jason Duval DTF-sticker",
    longDesc: "This Lucia Caminos | Jason Duval They are believed to be the main protagonists of the upcoming game Grand Theft Auto VI. DTF sticker (6 × 4 cm) reflects a bold, rebellious duo vibe.",
    originalPrice: 199,
    finalPrice: 29,
  },
  {
    id: "tr-4",
    category: "trending",
    src: "https://i.pinimg.com/originals/11/39/64/1139647b65167a1c2a9f5895b58083b2.gif",
    title: "Pocoyo Sticker",
    tag: "Trending",
    shortDesc: "Smooth loop animation.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-5",
    category: "trending",
    src: "https://i.pinimg.com/736x/70/3e/57/703e57fd0dabe23ab3dd55c719db8fad.jpg",
    title: "SPIDER-MAN",
    tag: "Trending",
    badge: "New",
    shortDesc: "A high-quality DTF sticker featuring Spider-Man relaxing on a Porsche GT3 RS",
    longDesc: "A high-quality DTF sticker featuring Spider-Man relaxing on a Porsche GT3 RS, blending superhero energy with luxury car culture.",
    originalPrice: 199,
    finalPrice: 29,
  },
  {
    id: "tr-6",
    category: "trending",
    src: "https://i.pinimg.com/1200x/c0/f5/e1/c0f5e140d779796930a1a9d844266e83.jpg",
    title: "ff logo sticker",
    tag: "Trending",
    shortDesc: "Smooth loop animation.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-7",
    category: "trending",
    src: "https://i.pinimg.com/1200x/00/a4/94/00a494141a36a87a4ecad3865592c8b4.jpg",
    title: "Animated Aesthetic GIF",
    tag: "Trending",
    shortDesc: "Smooth loop animation.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-8",
    category: "trending",
    src: "https://i.pinimg.com/736x/a2/ff/6c/a2ff6cef71615787f49719fb70d7a085.jpg",
    title: "bgmi ",
    tag: "Trending",
    badge: "New",
    shortDesc: "Smooth loop animation.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-9",
    category: "trending",
    src: "https://i.pinimg.com/736x/cf/cb/e2/cfcbe23a1a3473986975bfb6917eede4.jpg",
    title: "Animated Aesthetic GIF",
    tag: "Trending",
    shortDesc: "Smooth loop animation.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-10",
    category: "trending",
    src: "https://i.pinimg.com/1200x/b2/ba/fb/b2bafb418b82abb211bde55889125d16.jpg",
    title: "Luxury Car Sticker",
    tag: "Trending",
    shortDesc: "Premium glossy design.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-11",
    category: "trending",
    src: "https://i.pinimg.com/736x/fc/ef/c9/fcefc989fa89ae548289ddab0f1a95f9.jpg",
    title: "GTA_VI sticker",
    tag: "Trending",
    shortDesc: "Premium glossy design.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-12",
    category: "trending",
    src: "https://i.pinimg.com/1200x/46/ce/66/46ce66226bbc2f1a6548d6336ba35574.jpg",
    title: "GTA_VI sticker",
    tag: "Trending",
    shortDesc: "Premium glossy design.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-13",
    category: "trending",
    src: "https://i.pinimg.com/736x/01/a8/49/01a8493f780df91ba316725687125de9.jpg",
    title: "bgmi sticker",
    tag: "Trending",
    badge: "Hot",
    shortDesc: "Premium glossy design.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-14",
    category: "trending",
    src: "https://i.pinimg.com/736x/bf/06/5f/bf065f947e4ac33b6167f70df406aa05.jpg",
    title: "bgmi sticker",
    tag: "Trending",
    shortDesc: "Premium glossy design.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-15",
    category: "trending",
    src: "https://i.pinimg.com/736x/30/43/39/3043396190a3712eb66157b0bb12ec2d.jpg",
    title: "ps5 girl",
    tag: "Trending",
    shortDesc: "Premium glossy design.",
    longDesc: "A unique animated sticker-style GIF that expresses aesthetic vibes and emotions.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-16",
    category: "trending",
    src: "https://i.pinimg.com/1200x/5a/ad/0a/5aad0a8534ff652ffab1788930547741.jpg",
    title: "Aesthetic Girl Art",
    tag: "Trending",
    shortDesc: "Premium digital artwork.",
    longDesc: "Trending aesthetic girl illustration perfect for covers, wallpapers, and designs.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-17",
    category: "trending",
    src: "https://i.pinimg.com/736x/18/8f/b0/188fb05fce96365ea65807e8ee65a159.jpg",
    title: "rockstar",
    tag: "Trending",
    badge: "New",
    shortDesc: "Premium digital artwork.",
    longDesc: "Trending aesthetic girl illustration perfect for covers, wallpapers, and designs.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-18",
    category: "trending",
    src: "https://i.pinimg.com/736x/c6/3f/f5/c63ff5143c6919ff422f735bab715dc3.jpg",
    title: "Aesthetic Girl Art",
    tag: "Trending",
    badge: "Hot",
    shortDesc: "Premium digital artwork.",
    longDesc: "Trending aesthetic girl illustration perfect for covers, wallpapers, and designs.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-19",
    category: "trending",
    src: "https://i.pinimg.com/736x/48/ba/91/48ba918c7fbdeee6056a7e664f35481b.jpg",
    title: "Dark Mood Anime Art",
    tag: "Trending",
    shortDesc: "Stylish anime design.",
    longDesc: "Represents deep emotions and mood expression. Very trending among youth.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-20",
    category: "trending",
    src: "https://i.pinimg.com/1200x/8b/0c/14/8b0c14adee9fc943bc139a125a0fc400.jpg",
    title: "Moody Girl Illustration",
    tag: "Trending gta",
    badge: "trending",
    shortDesc: "Minimal clean art.",
    longDesc: "Soft shades and emotional aesthetic artwork, suitable for any theme.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-21",
    category: "trending",
    src: "https://i.pinimg.com/736x/15/64/6c/15646ca71fc337aee3307db372b6e767.jpg",
    title: "Moody Girl Illustration",
    tag: "Trending",
    shortDesc: "Minimal clean art.",
    longDesc: "Soft shades and emotional aesthetic artwork, suitable for any theme.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-22",
    category: "trending",
    src: "https://i.pinimg.com/1200x/c5/aa/38/c5aa38ad698123fa88b13b02bcf90c12.jpg",
    title: "Moody Girl Illustration",
    tag: "Trending",
    shortDesc: "Minimal clean art.",
    longDesc: "Soft shades and emotional aesthetic artwork, suitable for any theme.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-23",
    category: "trending",
    src: "https://i.pinimg.com/1200x/80/2c/6d/802c6df0611780efb775913bc18ad73a.jpg",
    title: "Cool Girl Modern Look",
    tag: "Trending",
    badge: "hot",
    shortDesc: "Trendy fashion artwork.",
    longDesc: "Represents confidence, style, and modern youth culture.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-24",
    category: "trending",
    src: "https://i.pinimg.com/736x/c5/31/c3/c531c3a450cb52d496b132bea6dee5fb.jpg",
    title: "Cute Cat Artwork",
    tag: "Trending",
    shortDesc: "Adorable cute-style art.",
    longDesc: "Perfect for diaries, laptops, and minimal art lovers.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-25",
    category: "trending",
    src: "https://i.pinimg.com/736x/59/25/04/5925040a9c9e5814e66af9e93ffb31ff.jpg",
    title: "Sad Anime Boy",
    tag: "Trending",
    shortDesc: "Emotional character.",
    longDesc: "Expresses relatable emotions through clean anime visual style.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-26",
    category: "trending",
    src: "https://i.pinimg.com/1200x/ab/20/0e/ab200eb1923996098bef2c5b6e49d029.jpg",
    title: "Cool Character Art",
    tag: "Trending",
    shortDesc: "Stylish detailed character.",
    longDesc: "Trending among teens for its bold aesthetic and expression.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-27",
    category: "trending",
    src: "https://i.pinimg.com/736x/89/6a/10/896a104d3a2e14bc3b0c022e5d5868ce.jpg",
    title: "Cool Character Art",
    tag: "Trending",
    badge: "New",
    shortDesc: "Stylish detailed character.",
    longDesc: "Trending among teens for its bold aesthetic and expression.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-28",
    category: "trending",
    src: "https://i.pinimg.com/736x/01/c2/ad/01c2ad7a7c93fc6508ed6753daf04455.jpg",
    title: "Vibes Graphic Art",
    tag: "Trending",
    shortDesc: "Aesthetic minimalist design.",
    longDesc: "Represents personal vibes and modern aesthetic energy.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-29",
    category: "trending",
    src: "https://i.pinimg.com/736x/91/a2/1e/91a21e1ae32dd8f7d353f13bcac7cd07.jpg",
    title: "Cool Character Art",
    tag: "Trending",
    shortDesc: "Stylish detailed character.",
    longDesc: "Trending among teens for its bold aesthetic and expression.",
    originalPrice: 149,
    finalPrice: 99,
  },
  {
    id: "tr-30",
    category: "trending",
    src: "https://i.pinimg.com/736x/66/ed/1e/66ed1e50fa8bd9a5924d2cafacf34b5f.jpg",
    title: "Cool Character Art",
    tag: "Trending",
    shortDesc: "Stylish detailed character.",
    longDesc: "Trending among teens for its bold aesthetic and expression.",
    originalPrice: 149,
    finalPrice: 99,
  },
];

export const posterData = [
  {
    id: "pos-1",
    category: "posters",
    src: "https://i.pinimg.com/originals/5e/52/03/5e5203bc54a8b079e74b99f79a282317.gif",
    title: "Retro Motion Poster",
    tag: "Poster",
    shortDesc: "Animated retro-style poster.",
    longDesc: "High-quality retro animation poster ideal for room walls, studios and aesthetic décor.",
    originalPrice: 349,
    finalPrice: 299,
  },
  {
    id: "pos-2",
    category: "posters",
    src: "https://i.pinimg.com/1200x/45/92/05/459205eebadf7b7d58be92bb1286a971.jpg",
    title: "Vintage Art Poster",
    tag: "Poster",
    shortDesc: "Classic art-theme poster.",
    longDesc: "Perfect wall-art poster reflecting vintage culture, style, and personality expression.",
    originalPrice: 299,
    finalPrice: 249,
  },
  {
    id: "pos-3",
    category: "posters",
    src: "https://i.pinimg.com/736x/60/2c/d7/602cd7ebc5cd8c71aef67b4338fb1997.jpg",
    title: "Moody Character Poster",
    tag: "Poster",
    shortDesc: "High-resolution character art.",
    longDesc: "A premium moody poster with expressive details ideal for aesthetic lovers.",
    originalPrice: 249,
    finalPrice: 199,
  },
  {
    id: "pos-4",
    category: "posters",
    src: "https://i.pinimg.com/1200x/13/43/a4/1343a43b4cc2701abdebead55c908a63.jpg",
    title: "Cyberpunk Poster",
    tag: "Poster",
    shortDesc: "Futuristic neon artwork.",
    longDesc: "Perfect for room décor, gamers, coders and sci-fi lovers showing futuristic vibes.",
    originalPrice: 349,
    finalPrice: 299,
  },
  {
    id: "pos-5",
    category: "posters",
    src: "https://i.pinimg.com/736x/52/36/6e/52366e37d38373ea59177a52a356c4f6.jpg",
    title: "Aesthetic Girl Poster",
    tag: "Poster",
    shortDesc: "Elegant portrait art.",
    longDesc: "Soft aesthetic poster crafted for calm energy, personality and modern expression.",
    originalPrice: 299,
    finalPrice: 249,
  },
  {
    id: "pos-6",
    category: "posters",
    src: "https://i.pinimg.com/1200x/62/d7/ff/62d7ffd8a9e5f0991af281abe8c94f7d.jpg",
    title: "Dark Themed Poster",
    tag: "Poster",
    shortDesc: "Bold dramatic artwork.",
    longDesc: "Ideal for aesthetic wall setups showcasing strong, bold and emotional tones.",
    originalPrice: 299,
    finalPrice: 249,
  },
  {
    id: "pos-7",
    category: "posters",
    src: "https://i.pinimg.com/736x/38/cc/af/38ccaf73cc226ac98d1f7c653af2ca86.jpg",
    title: "Minimal Character Poster",
    tag: "Poster",
    shortDesc: "Clean minimal art.",
    longDesc: "Simple, clean and modern poster suitable for any wall aesthetic or workspace.",
    originalPrice: 249,
    finalPrice: 199,
  },
  {
    id: "pos-8",
    category: "posters",
    src: "https://i.pinimg.com/1200x/81/5c/62/815c62fa8e4fe8404a7ae60c3e3d1a02.jpg",
    title: "Creative Portrait Poster",
    tag: "Poster",
    shortDesc: "Premium artistic design.",
    longDesc: "Crafted for art lovers who enjoy storytelling visuals and expressive artworks.",
    originalPrice: 299,
    finalPrice: 249,
  },
  {
    id: "pos-9",
    category: "posters",
    src: "https://i.pinimg.com/736x/5d/58/23/5d582381b3707b973548167a5aa99d80.jpg",
    title: "Modern Art Poster",
    tag: "Poster",
    shortDesc: "Trendy new-gen poster.",
    longDesc: "A stylish modern poster representing bold themes, emotions and individuality.",
    originalPrice: 249,
    finalPrice: 199,
  },
  {
    id: "pos-10",
    category: "posters",
    src: "https://i.pinimg.com/736x/cf/a3/ae/cfa3ae0c9a718fea7295e7f07887c38e.jpg",
    title: "Cool Aesthetic Poster",
    tag: "Poster",
    shortDesc: "Balance of vibes & style.",
    longDesc: "Perfect for modern rooms, offices and personality-driven wall decoration.",
    originalPrice: 299,
    finalPrice: 249,
  },
];

export const goodiesData = [
  {
    id: "gies-1",
    category: "goodies",
    src: "https://i.pinimg.com/736x/63/26/50/632650d4a548158dfc3120f93ca95ce1.jpg",
    title: "Premium Goodies Pack",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "gies-2",
    category: "goodies",
    src: Prgg,
    title: "Premium Printed Goodie",
    tag: "Goodies",
    shortDesc: "High-quality printed design.",
    longDesc: "Perfect for gifting and personal use. Premium glossy finish for aesthetic lovers.",
    originalPrice: 169,
    finalPrice: 129,
  },
  {
    id: "gies-3",
    category: "goodies",
    src: "https://i.pinimg.com/736x/d4/0d/c6/d40dc63c022f44a6b1933887c058cc12.jpg",
    title: "Cute Aesthetic Goodie",
    tag: "Goodies",
    shortDesc: "Glossy finish aesthetic.",
    longDesc: "Perfect for custom diaries, phone covers, laptops, and decor lovers.",
    originalPrice: 129,
    finalPrice: 99,
  },
  {
    id: "gies-4",
    category: "goodies",
    src: "https://i.pinimg.com/1200x/d6/d1/93/d6d193915dc45cf446b9c16627d278da.jpg",
    title: "Moody Art Goodie",
    tag: "Goodies",
    shortDesc: "High-resolution art.",
    longDesc: "Ideal for those who love expressive art and stylish mood-based designs.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "gies-5",
    category: "goodies",
    src: "https://i.pinimg.com/736x/1f/b1/71/1fb17194657e89aa4b1d4b6608fab6c8.jpg",
    title: "Moody Art Goodie",
    tag: "Goodies",
    shortDesc: "High-resolution art.",
    longDesc: "Ideal for those who love expressive art and stylish mood-based designs.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "gies-6",
    category: "goodies",
    src: "https://i.pinimg.com/1200x/d0/69/62/d0696284e5a0ecf0ed79af0675331ca6.jpg",
    title: "Moody Art Goodie",
    tag: "Goodies",
    shortDesc: "High-resolution art piece.",
    longDesc: "Created for moody aesthetic lovers wanting expressive modern designs.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "gies-7",
    category: "goodies",
    src: "https://i.pinimg.com/1200x/21/de/40/21de40edac16bc9a68a415e3c9592914.jpg",
    title: "gaming mouse ",
    tag: "Goodies",
    shortDesc: "High-resolution art piece.",
    longDesc: "Created for moody aesthetic lovers wanting expressive modern designs.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "gies-8",
    category: "goodies",
    src: "https://i.pinimg.com/736x/28/9c/58/289c58da0cc815dbd60cfcd8b074767a.jpg",
    title: "Moody Art Goodie",
    tag: "Goodies",
    shortDesc: "High-resolution art piece.",
    longDesc: "Created for moody aesthetic lovers wanting expressive modern designs.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "gies-9",
    category: "goodies",
    src: "https://i.pinimg.com/474x/41/a0/4d/41a04d367208f28552c0fc4ae77acffd.jpg",
    title: "Soft Character Accessory",
    tag: "Goodies",
    shortDesc: "Minimal character design.",
    longDesc: "Aesthetic and soft-toned accessory for modern youth expression.",
    originalPrice: 169,
    finalPrice: 129,
  },
  {
    id: "gies-10",
    category: "goodies",
    src: "https://i.pinimg.com/1200x/19/6e/53/196e53b9025f6deb4427108b3b5bd894.jpg",
    title: "Modern Art Goodie",
    tag: "Goodies",
    shortDesc: "Premium modern artwork.",
    longDesc: "Perfect for aesthetic room décor and stylish expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
];

export const funnyData = [
  {
    id: "funn-1",
    category: "funny",
    src: "https://i.pinimg.com/736x/ff/0e/f9/ff0ef9d4322a62845183e672c007999c.jpg",
    title: "Karan Johar funny",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "funn-2",
    category: "funny",
    src: "https://i.pinimg.com/1200x/60/c1/68/60c168950c64000e4fed405ae5bc1f44.jpg",
    title: "gopi ben funny",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "funn-3",
    category: "funny",
    src: "https://tinopia.com/wp-content/uploads/2025/08/hakla-shahrukh-khan-sticker.webp",
    title: "Shahrukh Khan funny",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "funn-4",
    category: "funny",
    src: "https://i.pinimg.com/736x/be/02/4a/be024a1d9fbb0fd493ade2bd56b351c5.jpg",
    title: "Akshay Kumar funny",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
];


export const hotData = [
  {
    id: "hot-1",
    category: "hot",
    src: "https://i.pinimg.com/736x/f0/59/51/f059514c3cd36212154edfc8feac2bfe.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-2",
    category: "hot",
    src: "https://i.pinimg.com/736x/39/4f/08/394f084b4b68e34313b385b40c823620.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-3",
    category: "hot",
    src: "https://i.pinimg.com/736x/b4/57/d8/b457d84ad71d92a3e219ac46c8628ffb.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-4",
    category: "hot",
    src: "https://i.pinimg.com/1200x/5b/eb/f7/5bebf76d08ebf00d8a677ef9930b68f5.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-5",
    category: "hot",
    src: "https://i.pinimg.com/736x/3c/de/b6/3cdeb6ad25bcdaf92786ed2fa548b69d.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-6",
    category: "hot",
    src: "https://i.pinimg.com/736x/92/11/a6/9211a603952afc1ceac9a7cf7984c28c.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-7",
    category: "hot",
    src: "https://i.pinimg.com/736x/7e/c4/bb/7ec4bbee6dae600bb70ef5545b209f15.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-8",
    category: "hot",
    src: "https://i.pinimg.com/736x/53/2f/bd/532fbd086173572192e9ea73eb6c4a3f.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-9",
    category: "hot",
    src: "https://i.pinimg.com/736x/0f/39/cf/0f39cf72386455226f16eaffff66ae1c.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-10",
    category: "hot",
    src: "https://i.pinimg.com/736x/6c/17/bb/6c17bb95646e58a9685a0b4188fb0ee5.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-11",
    category: "hot",
    src: "https://i.pinimg.com/736x/03/29/dd/0329dde8f850c98123d99eebfd8a7f9f.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-12",
    category: "hot",
    src: "https://i.pinimg.com/736x/12/f4/1d/12f41d2b13ab214af77697de08a0fd21.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-13",
    category: "hot",
    src: "https://i.pinimg.com/736x/50/e2/ad/50e2ad259f7855b4a362eb75bd63f42d.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-14",
    category: "hot",
    src: "https://i.pinimg.com/736x/bd/71/04/bd7104ec42a28e5f08b2caabc4bc46e0.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-15",
    category: "hot",
    src: "https://i.pinimg.com/736x/a1/34/e5/a134e5312c64ca55abc7c59d55bf94ef.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-16",
    category: "hot",
    src: "https://i.pinimg.com/736x/bd/71/04/bd7104ec42a28e5f08b2caabc4bc46e0.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-17",
    category: "hot",
    src: "https://i.pinimg.com/736x/f6/7d/d8/f67dd89970431c40baf80db3a9db87e2.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-18",
    category: "hot",
    src: "https://i.pinimg.com/736x/e3/b6/c9/e3b6c9ce3dccf23e1021cbf661b25213.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
  {
    id: "hot-19",
    category: "hot",
    src: "https://i.pinimg.com/736x/a8/6f/54/a86f54fe53d6f467c5c6e085b27e01fd.jpg",
    title: "Honies sticker ",
    tag: "Goodies",
    shortDesc: "Exclusive custom design.",
    longDesc: "A premium goodies item crafted for unique style, vibes, and personal expression.",
    originalPrice: 199,
    finalPrice: 149,
  },
];

export const learningProducts = [
  {
    id: "lrnkids-1",
    category: "learning-products",
    title: "My Cute Board Book – Vegetables (Waterproof Edition)",
    shortDesc:
      "Specially designed for toddlers to learn vegetable names in a fun way.",
    longDesc:
      "My Cute Board Book – Vegetables is specially designed for toddlers to learn vegetable names in a fun way. It comes with hard board pages that are strong and long-lasting. The waterproof material makes it safe from spills and rough use. Its compact 12×12 cm size is perfect for little hands to hold easily. Bright pictures help improve vocabulary and early learning skills. 🌱📚",
    finalPrice: 79,
    originalPrice: 299,
    discount: 33,
    rating: 4.5,
    stock: 10,
    src: "https://i.pinimg.com/736x/7d/f5/ff/7df5ff2c4e6d86449f56effbe64c1c15.jpg",
    subImages: [
      "https://i.pinimg.com/736x/21/fc/74/21fc7494ca3b234f39ceda3a1448a3f8.jpg",
      "https://i.pinimg.com/474x/12/8e/c3/128ec3852edec5d67756262be8cfab79.jpg",
      "https://i.pinimg.com/736x/36/ce/52/36ce52e218df0b9e1cb555b0e5332e19.jpg"
    ],
    tags: ["Board book"],
  },
  {
    id: 1,
    title: "Alphabet Board Book",
    shortDesc: "Perfect book for toddlers to learn ABC.",
    longDesc:
      "This colorful alphabet board book helps children recognize letters with bright images and durable pages specially designed for small hands.",
    finalPrice: 299,
    originalPrice: 399,
    src: "https://i.pinimg.com/736x/3d/0a/31/3d0a316a90cc199b0cf78d3b0d1b295f.jpg"
  },
  {
    id: 2,
    title: "Animal Picture Book",
    shortDesc: "Fun animal learning book.",
    longDesc:
      "Kids can explore the animal world through vibrant illustrations that improve memory and recognition skills.",
    finalPrice: 249,
    originalPrice: 349,
    src: "https://i.pinimg.com/736x/ee/81/4e/ee814ed3908c65832b96e51dae1910d6.jpg"
  },
  {
    id: 3,
    title: "Numbers & Shapes Book",
    shortDesc: "Learn numbers and shapes easily.",
    longDesc:
      "An interactive board book that teaches numbers and shapes in a playful and engaging way.",
    finalPrice: 269,
    originalPrice: 369,
    src: "https://i.pinimg.com/736x/ea/09/5e/ea095e535d3562621a87cc91736a3e96.jpg"
  },
  {
    id: 4,
    title: "First Words Book",
    shortDesc: "Build vocabulary for toddlers.",
    longDesc:
      "Helps children identify common everyday objects and develop early speaking skills.",
    finalPrice: 199,
    originalPrice: 299,
    src: "https://i.pinimg.com/1200x/3b/e6/20/3be620eee32e3c523012f1e645c16da1.jpg"
  },
  {
    id: 5,
    title: "Alphabet Play Mat",
    shortDesc: "Soft foam alphabet mat.",
    longDesc:
      "Large alphabet mat made of safe foam material, perfect for indoor play and early learning.",
    finalPrice: 799,
    originalPrice: 999,
    src: "https://i.pinimg.com/1200x/60/34/da/6034da8925097f5ec71c629ba90ddad8.jpg"
  },
  {
    id: 6,
    title: "Animal World Play Mat",
    shortDesc: "Fun jungle themed mat.",
    longDesc:
      "A colorful jungle play mat designed to stimulate imagination and improve coordination skills.",
    finalPrice: 899,
    originalPrice: 1099,
    src: "https://i.pinimg.com/1200x/09/59/80/0959809c43cc41db9b5f8ae7807dbcdc.jpg"
  },
  {
    id: 7,
    title: "Numbers Foam Mat",
    shortDesc: "Learn numbers while playing.",
    longDesc:
      "Interlocking foam number mat that helps kids learn counting in an interactive way.",
    finalPrice: 749,
    originalPrice: 949,
    src: "https://i.pinimg.com/1200x/62/32/8c/62328cd0c0fa5361b4dbf165f8d68977.jpg"
  },
  {
    id: 8,
    title: "Shapes Puzzle Mat",
    shortDesc: "Colorful shape puzzle mat.",
    longDesc:
      "Enhances motor skills and shape recognition through interactive floor play.",
    finalPrice: 849,
    originalPrice: 1049,
    src: "https://i.pinimg.com/736x/f9/f8/f0/f9f8f081b517e70084a92f5880e53feb.jpg"
  },
  {
    id: 9,
    title: "Touch & Feel Book",
    shortDesc: "Sensory learning experience.",
    longDesc:
      "Textured pages that allow toddlers to touch and explore different materials safely.",
    finalPrice: 349,
    originalPrice: 449,
    src: "https://i.pinimg.com/736x/f1/25/e9/f125e9f5a699ad6186cb90464e9a583d.jpg"
  },
  {
    id: 10,
    title: "Flashcard Learning Kit",
    shortDesc: "Complete early learning kit.",
    longDesc:
      "Includes alphabet, number, and animal flashcards to boost memory and cognitive skills.",
    finalPrice: 399,
    originalPrice: 499,
    src: "https://i.pinimg.com/736x/b7/a2/bd/b7a2bd58e7115c1bdb98eaecc0fdcb42.jpg"
  }
];

export const stickerPacksData = [
  {
    id: "stk-pack-1",
    category: "stickers",
    title: "1st-ever Exclusive 10 DTF Sticker Pack",
    shortDesc: "Premium 3D-feel waterproof UV-DTF stickers featuring Porsche GT3 RS, GTA VI, Anime & more!",
    longDesc: "Level up your gear with our 1st ever exclusive 10-piece sticker drop! 🔥 Crafted with ultra-premium UV-DTF technology, these stickers aren't just regular prints—they are waterproof, scratch-resistant, and pack a vibrant, raised 3D-like texture. \n\nThis viral collection includes:\n• The beastly **Porsche GT3 RS**\n• The hyped **GTA VI Protagonist Couple**\n• The smashing **Angry Hulk Face**\n• Fan-favorite **Anime Characters** in ultimate drip\n• Plus more pop-culture and super-car icons!\n\nPerfect for laptops, phones, bikes, or any smooth surface. Just peel, stick, and flex your vibe!",
    finalPrice: 259,
    originalPrice: 499,
    discount: 48,
    rating: 4.8,
    stock: 15,
    wishlistCount: 0, 
    richContent: [
      // {
      //   heading: "See The Product in Action! 🎬",
      //   text: "Watch how premium these stickers look in real life.",
      //   // 🔥 YAHAN VIDEO LINK DAAL DIJIYE:
      //   videoUrl: "https://www.youtube.com/shorts/PFQzW2oez00?feature=share" 
      // },
      {
        heading: "Spider-Man on Porsche GT3 RS",
        text: "",
        // 🔥 YAHAN IMAGE LINK BHI DAAL SAKTE HAIN:
        image: "https://i.pinimg.com/1200x/8a/57/c6/8a57c6e9f49275ea3d83e6fc5238eacd.jpg"
      },
      {
        heading: "most viral GTA VI Protagonist Couple",
        text: "",
        // 🔥 YAHAN IMAGE LINK BHI DAAL SAKTE HAIN:
        image: "https://i.pinimg.com/1200x/de/1f/9d/de1f9d609a4fff4a52139e56fc67e89a.jpg"
      }
    ],
    src: "https://i.pinimg.com/736x/cb/06/9e/cb069e70b3e556abd90693efb343c87f.jpg", // Yahan apni original image URL daal dena
    subImages: [
      "https://i.pinimg.com/736x/cb/06/9e/cb069e70b3e556abd90693efb343c87f.jpg",
      "https://i.pinimg.com/736x/4f/9c/5c/4f9c5c6b17f2c27b0b2fbdb36b9db327.jpg",
      "https://i.pinimg.com/736x/bb/e3/dd/bbe3dd72c4aec824fa693e6ecf4f8c4b.jpg"
    ], // Yahan bhi apni images update kar lena
    tags: ["stickers", "dtf", "premium", "new-drop", "gta6", "anime", "porsche"],
    videoUrl: "https://www.youtube.com/shorts/PFQzW2oez00?feature=share", // Tumhara promo video
  },
];